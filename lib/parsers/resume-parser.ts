/**
 * Resume parser. Applies the EDA-measured cleaning rules, then a heuristic structured
 * parse (a deeper LLM parse is layered on in Module 1). Text extraction for uploaded
...
/** Heuristic baseline parse (EDA-cleaned). */
export function parseResume(raw: string): ParsedResume {
  ...
  const expText = sliceSection(cleaned, ["experience", "work experience", "professional experience"]);
  const eduText = sliceSection(cleaned, ["education", "academic background"]);
  ...
  const education = eduText
    ? eduText
        .split(/\n/)
        .map((l) => l.trim())
        .filter((l) => /\b(BSc|BA|BS|MSc|MS|MBA|PhD|Bachelor|Master|Degree|University|College)\b/i.test(l))
        .slice(0, 4)
        .map((l) => ({ degree: l }))
    : [];
